...
import argparse
from brag.cli import cmd_add, cmd_list, cmd_export
...
def test_cmd_add_simple(mock_storage):
    args = argparse.Namespace(content=["Test", "content"], tags=None, project=None, edit=False)
...
def test_cmd_add_with_metadata(mock_storage):
    args = argparse.Namespace(content=["Test"], tags=["tag1"], project="proj1", edit=False)
    cmd_add(args, mock_storage)
...
def test_cmd_add_empty_aborts(mock_storage):
    ...
    # Mock open_editor to return empty string if called (it shouldn't be called if content is empty and edit is False? 
    # Wait, logic says: if args.edit or not content: open_editor
    
    with patch("brag.cli.open_editor", return_value=""):
